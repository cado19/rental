...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($_POST['make'])) {
		$make_err = "Required";
		header("Location: index.php?page=client/vehicle/new&make=$make_err");
		exit;
	}
	if (empty($_POST['model'])) {
		$model_err = "Required";
		header("Location: index.php?page=client/vehicle/new&model_err=$model_err");
		exit;
	}

	if (empty($_POST['num_plate_1'])) {
		$num_plate_1_err = "Required";
		header("Location: index.php?page=client/vehicle/new&num_plate_1_err=$num_plate_1_err");
		exit;
	...
	if (empty($_POST['num_plate_2'])) {
		...
		header("Location: index.php?page=client/vehicle/new&num_plate_2_err=$num_plate_2_err");
		exit;
	...
	if (empty($_POST['category'])) {
		...
		header("Location: index.php?page=client/vehicle/new&category_err=$category_err");
		exit;
	}
	if (empty($_POST['transmission'])) {
		$transmission_err = "Required";
		header("Location: index.php?page=client/vehicle/new&transmission_err=$transmission_err");
		exit;
	}
	if (empty($_POST['fuel'])) {
		$fuel_err = "Required";
		header("Location: index.php?page=client/vehicle/new&fuel_err=$fuel_err");
	}

	if (empty($_POST['seats'])) {
		$seats_err = "Required";
		header("Location: index.php?page=client/vehicle/new&seats_err=$seats_err");
		exit;
	}

	if (empty($_POST['colour'])) {
		$colour_err = "Required";
		header("Location: index.php?page=client/vehicle/new&colour_err=$colour_err");
		exit;
	}
	...
	if (empty($_POST['daily_rate'])) {
		...
		header("Location: index.php?page=client/vehicle/new&daily_rate_err=$daily_rate_err");
		exit;
	...
	if (empty($_POST['vehicle_excess'])) {
		...
		header("Location: index.php?page=client/vehicle/new&vehicle_excess_err=$vehicle_excess_err");
		exit;
	} elseif (($_POST['vehicle_excess']) <= 0) {
		$vehicle_excess_err = "Must be greater than 0";
		header("Location: index.php?page=client/vehicle/new&vehicle_excess_err=$daily_rate_err");
		exit;
	}
	...
	if ($taken_plate == "Taken") {
		...
		header("Location: index.php?page=client/vehicle/new&err_msg=$number_plate_err");
		exit;
	}
	...
	if ($stmt->execute([$make, $model, $number_plate, $category, $transmission, $fuel, $seats, $drive_train, $colour, $partner_id])) {
		...
		header("Location: index.php?page=client/vehicle/new&msg=$res");
		exit;
	}
	...
	$sql1 = "INSERT INTO vehicle_pricing (vehicle_id,  daily_rate, vehicle_excess) VALUES (?,?,?)";
	$stmt1 = $con->prepare($sql1);
	if ($stmt1->execute([$res, $daily_rate, $vehicle_excess])) {
		$result = "Success";
	} else {
		$result = "Failed";
		header("Location: index.php?page=client/vehicle/new&err_msg=$result");
		exit;
	}
	...
	if ($stmt2->execute([$res, $bluetooth, $keyless_entry, $reverse_cam, $audio_input, $gps, $android_auto, $apple_carplay, $sunroof])) {
		...
		header("Location: index.php?page=client/vehicle/success&msg=$response");
		exit;
	} else {
		$response = "Failed";
		header("Location: index.php?page=client/vehicle/new&msg=$response&id=$partner_id");
		exit;
	}

	// $log->warning($response);

	// header("Location: index.php?page=fleet/new&msg=$response");
} else {
