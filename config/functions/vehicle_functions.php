...
function partner_vehicles($partner_id) {
	...
	try {
		...
		$sql = "SELECT vb.id, vb.make AS make, vb.model AS model, vb.number_plate, vb.category AS category, vp.daily_rate FROM vehicle_basics vb INNER JOIN vehicle_pricing vp ON vb.id = vp.vehicle_id INNER JOIN partners p ON vb.partner_id = p.id WHERE vb.partner_id = ?";
		$stmt = $con->prepare($sql);
