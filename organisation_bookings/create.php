<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if (empty($_POST['start_date'])) {
            ...
            header("Location: index.php?page=organisation_bookings/new&start_date_err=$start_date_err");
            exit;
        }
        ...
        // 	header("Location: index.php?page=organisation_bookings/new&start_time_err=$start_time_err");
        // 	exit;
        // }
        // if (empty($_POST['end_time'])) {
        // 	$end_time_err = "Required";
        // 	header("Location: index.php?page=organisation_bookings/new&end_time_err=$end_time_err");
        // 	exit;
        // }
        $a_id       = $_POST['account_id'];
        $v_id       = $_POST['vehicle_id'];
        $o_id       = $_POST['organisation_id'];
        ...
        $posts = [$v_id, $o_id, $d_id, $a_id, $start_date, $end_date, $start_time, $end_time];
        ...
        $result = save_organisation_booking($v_id, $o_id, $d_id, $a_id, $start_date, $end_date, $start_time, $end_time);
        if ($result == "No Success") {
            $err = "An error occured. Try again later";
            header("Location: index.php?page=organisation_bookings/new&err_msg=$err");
            exit;
        } else {
            //CREATE BOOKING NUMBER
            $no = "BO-" . str_pad($result, 3, "0", STR_PAD_LEFT);
            save_organisation_booking_number($result, $no);

            // CREATE A CONTRACT WITH THE CURRENT LAST BOOKING ID AS THE REFERENCE ID (THIS JUST MIGHT BE IN THE FUNCTIONS FILE)
            $response = create_organisation_contract($result);
            ...
            $total_res = update_organisation_booking($total, $result);

            //REDIRECT TO THE CONTRACT PAGE SO THAT A SIGNATURE CAN BE UPLOADED IF IT IS AVAILABLE
            $msg = "Booking created";

            header("Location: index.php?page=organisation_bookings/show&id=$result&msg=$msg");
        }
    } else {
