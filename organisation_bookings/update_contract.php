...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$sql = "UPDATE organisation_contracts SET signature = ?, status = ? WHERE id = ?";
	$stmt = $con->prepare($sql);
	$stmt->execute([$nama_file, $status, $new_id]);

	// fetch id of booking from contracts for redirect
	$sql1 = "SELECT organisation_booking_id FROM organisation_contracts WHERE id = ?";
	$stmt1 = $con->prepare($sql1);
	$stmt1->execute([$new_id]);
	$booking_id = $stmt1->fetch();

	// redirect to booking show page
	header("Location: ../index.php?page=organisation_bookings/show&id=" . $booking_id['booking_id']);
